#include "DSMySQLEntityData.h"
#include "DSMySQLQuery.h"

DynSoft::DSMySQLEntityData::DSMySQLEntityData(
	DSMySQLDatabase *db,
...
DynSoft::DSClassInfo DynSoft::DSMySQLEntityData::GetClassInfo() {
	return DynSoft::DSClassInfo(
		DS_MYSQL_ENTITY_DATA_NAME,
		DS_MYSQL_ENTITY_DATA_VERSION,
		DS_MYSQL_ENTITY_DATA_AUTHOR
	);
}

void DynSoft::DSMySQLEntityData::Get(DSMySQLDatabaseEntity *entity) {
	Get(entity, _N(""), _(""));
}

void DynSoft::DSMySQLEntityData::Get() {
...
void DynSoft::DSMySQLEntityData::Get(const unsigned int primaryKey) {
	DSQueryParser parser;
...
void DynSoft::DSMySQLEntityData::Get(const wxString &primaryKey, const bool delimiters) {
	DSQueryParser parser;
	parser.Prepare(_N("SELECT * FROM :table: WHERE :primaryKey: = :value:"));
	parser.Bind(_N("table"), table, false);
	parser.Bind(_N("primaryKey"), this->primaryKey, false);
...
void DynSoft::DSMySQLEntityData::Get(const wxString &field, const wxString &value, const bool delimiters) {
	DSQueryParser parser;
	if(orderByField.IsEmpty()) {
...
bool DynSoft::DSMySQLEntityData::Exists(
