#include "DSSQLiteEntityData.h"
#include "DSSQLiteQuery.h"

DynSoft::DSSQLiteEntityData::DSSQLiteEntityData(
	DSSQLiteDatabase *db,
...
DynSoft::DSClassInfo DynSoft::DSSQLiteEntityData::GetClassInfo() {
	return DynSoft::DSClassInfo(
		DS_SQLITE_ENTITY_DATA_NAME,
		DS_SQLITE_ENTITY_DATA_VERSION,
		DS_SQLITE_ENTITY_DATA_AUTHOR
	);
}

void DynSoft::DSSQLiteEntityData::Get(DSSQLiteDatabaseEntity *entity) {
	Get(entity, _N(""), _(""));
}

void DynSoft::DSSQLiteEntityData::Get() {
...
void DynSoft::DSSQLiteEntityData::Get(const unsigned int primaryKey) {
	DSQueryParser parser;
...
void DynSoft::DSSQLiteEntityData::Get(const wxString &primaryKey, const bool delimiters) {
	DSQueryParser parser;
	parser.Prepare(_N("SELECT * FROM :table: WHERE :primaryKey: = :value:"));
	parser.Bind(_N("table"), table, false);
	parser.Bind(_N("primaryKey"), this->primaryKey, false);
...
void DynSoft::DSSQLiteEntityData::Get(const wxString &field, const wxString &value, const bool delimiters) {
	DSQueryParser parser;
	if(orderByField.IsEmpty()) {
...
bool DynSoft::DSSQLiteEntityData::Exists(
